...
function hdALRecordExternal(label,note=''){
 const rows=hdALLoad();rows.unshift({id:crypto.randomUUID?crypto.randomUUID():Date.now()+'-'+Math.random().toString(16).slice(2),at:Date.now(),actionId:'external',label:String(label||'自動記録'),note:String(note||''),changes:[],undone:false,external:true});hdALSave(rows);hdALRender();
}
...
function hdALRender(){
 ...
 host.innerHTML=`<div class="section-head"><div><div class="eyebrow">ONE-TAP LOG</div><h2>任務連動・活動ログ</h2></div><span class="muted">受注中だけ自動反映</span></div><div class="hd-al-note">ゲーム側の任務受注状態は取得できないため、<b>任務DBからチェックリストに追加した未完了任務だけ</b>進捗へ反映するよ。任務の出現前・未受注状態で先にカウントしてしまうのを防ぐ仕組み。</div><div class="hd-al-toolbar">${groups.map(g=>`<button type="button" class="ghost small${g===hdALGroup?' active':''}" data-hd-al-group="${hdALEsc(g)}">${hdALEsc(g)}</button>`).join('')}<button type="button" class="ghost small" data-hd-al-quests>受注任務を管理</button></div><div class="hd-al-grid">${actions.map(a=>{const n=hdALAcceptedCount(a);return `<button type="button" class="hd-al-action" data-hd-al-action="${a.id}"><span class="hd-al-icon">${a.icon}</span><span><b>${hdALEsc(a.label)}</b><small>${hdALEsc(a.note)}</small></span><em>${n?`反映 ${n}件`:'記録のみ'}</em></button>`}).join('')}</div><div class="hd-al-history"><div class="hd-al-history-head"><strong>直近の活動</strong><span>${logs.length}件</span></div>${logs.length?logs.map(x=>`<div class="hd-al-log${x.undone?' undone':''}${x.external?' external':''}"><div><b>${hdALEsc(x.label)}</b><small>${new Date(x.at).toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'})}${x.external?(x.note?`・${hdALEsc(x.note)}`:'・自動記録'):`・${x.changes?.length?`任務 ${x.changes.length}件反映`:'任務反映なし'}${x.undone?'・取消済み':''}`}</small></div>${x.external?'<span class="hd-al-auto">自動記録</span>':`<button type="button" class="ghost small" data-hd-al-undo="${x.id}" ${x.undone?'disabled':''}>1手戻す</button>`}</div>`).join(''):'<div class="empty">まだ活動ログはないよ</div>'}</div>`;
}
...
window.addEventListener('load',()=>setTimeout(()=>{hdALEnsure();hdALRender()},1200));setTimeout(hdALEnsure,1600);window.addEventListener('storage',()=>setTimeout(hdALRender,0));window.addEventListener('hd:procurement-progress',e=>{const d=e.detail||{};if(!d.complete)return;hdALRecordExternal(`装備調達完了: ${d.target||d.wanted||'装備'}`,`所持 ${Number(d.owned)||0} / 必要 ${Number(d.needed)||0}`)});