...
function renderEquipment(){
 const el=document.getElementById('equipmentList');if(!el)return;const q=(document.getElementById('equipmentSearch')?.value||'').toLowerCase(),all=hdLoad(HD_EQUIP_KEY,[]),view=hdEquipLedgerViewLoad(),sourceFilter=['all','sync','manual','plan'].includes(view.sourceFilter)?view.sourceFilter:'all';
 const sourceKind=x=>x?.source==='kancolle-import'?'sync':x?.source==='equipment-plan'?'plan':'manual',sourceOk=x=>sourceFilter==='all'||sourceKind(x)===sourceFilter;
 const sourceCounts={all:all.length,sync:all.filter(x=>sourceKind(x)==='sync').length,manual:all.filter(x=>sourceKind(x)==='manual').length,plan:all.filter(x=>sourceKind(x)==='plan').length};
 const sourceHost=document.getElementById('equipmentSourceFilters');if(sourceHost)sourceHost.innerHTML=[['all','すべて'],['sync','同期済み'],['manual','手動'],['plan','計画']].map(([v,label])=>`<button type="button" class="guide-chip ${v===sourceFilter?'active':''}" data-eq-source="${v}">${label} ${sourceCounts[v]}</button>`).join('');
 const rows=all.filter(x=>sourceOk(x)&&(!q||`${x.name} ${x.category} ${x.assigned} ${x.memo}`.toLowerCase().includes(q)));
 ...
 el.innerHTML=rows.length?rows.map(x=>{const source=sourceKind(x),sourceBadge=source==='sync'?'<span class="equipment-source-badge sync">ゲーム同期</span>':source==='plan'?'<span class="equipment-source-badge plan">計画</span>':'<span class="equipment-source-badge manual">手動</span>';return `<article class="advanced-card"><div class="advanced-card-head"><div><strong>${hdEsc(x.name)}</strong><div class="equipment-card-meta"><span class="muted">${hdEsc(x.category||'カテゴリ未設定')} ・ 所持 ${Number(x.count)||0}${x.source==='equipment-plan'?' ・ 計画保存':''}</span>${sourceBadge}</div></div><div class="mini-actions"><button class="ghost small" data-eq-edit="${x.id}">編集</button><button class="ghost small" data-eq-delete="${x.id}">削除</button></div></div><div class="progress-line"><span>改修 ★${Number(x.star)||0} → 目標 ★${Number(x.targetStar)||0}</span><progress max="10" value="${Math.min(10,Number(x.star)||0)}"></progress></div>${x.assigned?`<div class="advanced-meta"><b>搭載:</b> ${hdEsc(x.assigned)}</div>`:''}${x.memo?`<p>${hdEsc(x.memo)}</p>`:''}</article>`}).join(''):empty;
}
...
function bindAdvancedEvents(){
 ...
 if(e.target.closest('[data-empty-add-equipment]')){openEquipment();return}
  if(e.target.closest('[data-eq-search-clear]')){const input=document.getElementById('equipmentSearch');if(input)input.value='';hdEquipLedgerViewSave({query:''});renderEquipment();return}
  if(e.target.closest('[data-empty-eq-reset]')){const input=document.getElementById('equipmentSearch');if(input)input.value='';hdEquipLedgerViewSave({query:'',sourceFilter:'all'});renderEquipment();return}
  const eqSource=e.target.closest('[data-eq-source]');if(eqSource){hdEquipLedgerViewSave({sourceFilter:eqSource.dataset.eqSource||'all'});renderEquipment();return}
