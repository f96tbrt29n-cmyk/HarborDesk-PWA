...
function snapshotResources(){
 ...
 const row={id:hdUid(),at:Date.now(),fuel:Number(res.fuel)||0,ammo:Number(res.ammo)||0,steel:Number(res.steel)||0,bauxite:Number(res.bauxite)||0};const h=hdLoad(HD_RESOURCE_HISTORY_KEY,[]);h.unshift(row);hdSave(HD_RESOURCE_HISTORY_KEY,h.slice(0,120));renderResourceHistory();renderDashboard();
}
...
async function importBackup(file){try{const obj=JSON.parse(await file.text());if(!obj?.localStorage)throw new Error();Object.entries(obj.localStorage).forEach(([k,v])=>{if(k.startsWith('harbordesk')&&typeof v==='string')localStorage.setItem(k,v)});alert('バックアップを復元したよ。画面を再読み込みするね。');location.reload()}catch{alert('HarborDeskのバックアップJSONを読み込めなかったよ')}}
...
function bindAdvancedEvents(){
 ...
 document.addEventListener('click',e=>{const ee=e.target.closest('[data-eq-edit]');if(ee){openEquipment(hdLoad(HD_EQUIP_KEY,[]).find(x=>x.id===ee.dataset.eqEdit));return}const ed=e.target.closest('[data-eq-delete]');if(ed){hdSave(HD_EQUIP_KEY,hdLoad(HD_EQUIP_KEY,[]).filter(x=>x.id!==ed.dataset.eqDelete));renderEquipment();renderDashboard();return}const he=e.target.closest('[data-history-delete]');if(he){hdSave(HD_RESOURCE_HISTORY_KEY,hdLoad(HD_RESOURCE_HISTORY_KEY,[]).filter(x=>x.id!==he.dataset.historyDelete));renderResourceHistory();return}const ev=e.target.closest('[data-event-edit]');if(ev){openEvent(hdLoad(HD_EVENT_KEY,[]).find(x=>x.id===ev.dataset.eventEdit));return}const dv=e.target.closest('[data-event-delete]');if(dv){hdSave(HD_EVENT_KEY,hdLoad(HD_EVENT_KEY,[]).filter(x=>x.id!==dv.dataset.eventDelete));renderEvents();renderDashboard()}});
