...
window.hdPHCreateSnapshot=hdPHCreateSnapshot;
...
async function hdPHAutoSnapshot(){try{const rows=await hdPHGetSnapshots();if(!rows.length||Date.now()-rows[0].at>20*3600000){const create=window.hdPHCreateSnapshot;if(typeof create!=='function')return false;return await create('自動（日次）')}return true}catch{return false}}
