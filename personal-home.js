...
function hdPHExternalText(){
 ...
 let syncAt=0;try{const sync=JSON.parse(localStorage.getItem('harbordesk-kancolle-sync-v1')||'null'),marker=Math.max(0,Number(localStorage.getItem('harbordesk-kancolle-last-success-at-v1'))||0);syncAt=typeof hdKcSyncSuccessAt==='function'?hdKcSyncSuccessAt(sync):Math.max(marker,Math.max(0,Number(sync?.lastSuccessAt)||0),Math.max(0,Number(sync?.syncedAt)||0))}catch{}
...
async function hdPHAutoSnapshot(){const rows=await hdPHGetSnapshots();if(rows.length&&Date.now()-rows[0].at<=20*3600000)return true;if(typeof hdPHCreateSnapshot!=='function')return false;return await hdPHCreateSnapshot('自動（日次）')}
