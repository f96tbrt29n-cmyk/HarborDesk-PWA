...
function hdPHResourceSummary(){
 ...
 ].map(x=>({...x,number:(x.value===''||x.value==null||!Number.isFinite(Number(x.value)))?null:Math.max(0,Number(x.value))}));
 const savedAt=Math.max(0,Number(stateResources.savedAt)||0,Number(materials.syncedAt)||0);
 return {rows,savedAt,hasAny:rows.some(x=>x.number!=null)};
}
