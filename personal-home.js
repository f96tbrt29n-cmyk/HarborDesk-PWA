...
function hdPHInstallBackupHooks(){
 ...
 if(typeof oldImport==='function'){window.importBackup=async function(file){await hdPHCreateSnapshot('外部復元直前');return oldImport(file)};const input=document.getElementById('importBackup');if(input)input.onchange=e=>{const f=e.target.files?.[0];if(f)window.importBackup(f)}}
}
