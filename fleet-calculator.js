...
function hdFCFleets(map){try{return typeof loadCustomFleets==='function'?(loadCustomFleets()[map]||[]):[]}catch{return []}}
function hdFCSelection(map){const fleets=hdFCFleets(map);let s={};try{s=JSON.parse(localStorage.getItem(HD_FC_SELECT_KEY)||'{}')||{}}catch{}return fleets.find(x=>x.id===s[map])?.id||fleets[0]?.id||'manual'}
function hdFCSetSelection(map,id){let s={};try{s=JSON.parse(localStorage.getItem(HD_FC_SELECT_KEY)||'{}')||{}}catch{}s[map]=id;localStorage.setItem(HD_FC_SELECT_KEY,JSON.stringify(s));hdFCRender()}
function hdFCMapKey(map,fleetId){return `${map}:${fleetId||'manual'}`}
function hdFCDefaultState(map,fleetId){const fleet=hdFCFleets(map).find(x=>x.id===fleetId);const ships=(fleet?.ships||[]).filter(x=>String(x.ship||'').trim());const coef=Number((typeof HD_MAP_ADVANCED_DATA!=='undefined'?HD_MAP_ADVANCED_DATA[map]?.los?.coef:null))||1;return {hqLevel:120,branchCoef:coef,enemyAir:0,shipCount:ships.length||6,ships:Array.from({length:Math.max(6,ships.length)},(_,i)=>({name:ships[i]?.ship||'',los:0})),gear:[{name:'',slot:0,star:0,maxProf:false}]}}
...
function hdFCEquipOptions(selected=''){const rows=hdFCCatalog().filter(x=>HD_FC_AIR_CATEGORIES.has(x.category)||Number(x.stats?.索敵)>0);const groups=new Map();for(const x of rows){const cat=x.category||'その他';if(!groups.has(cat))groups.set(cat,[]);groups.get(cat).push(x)}return `<option value="">未設定</option>`+[...groups].map(([cat,items])=>`<optgroup label="${hdFCEsc(cat)}">${items.sort((a,b)=>a.name.localeCompare(b.name,'ja')).map(x=>`<option value="${hdFCEsc(x.name)}" ${x.name===selected?'selected':''}>${hdFCEsc(x.name)}</option>`).join('')}</optgroup>`).join('')}
...
function hdFCMapEnemyAirCandidates(map){const vals=new Set();const addText=t=>{for(const m of String(t||'').matchAll(/敵制空(?:値)?\s*(\d+)/g))vals.add(Number(m[1]))};try{const nodes=typeof HD_NODE_PATTERN_DATA!=='undefined'?HD_NODE_PATTERN_DATA[map]||{}:{};Object.values(nodes).forEach(n=>(n.patterns||[]).forEach(p=>addText(p.air)))}catch{}try{const ov=typeof HD_NODE_DETAIL_OVERRIDES!=='undefined'?HD_NODE_DETAIL_OVERRIDES[map]||{}:{};Object.values(ov).forEach(n=>{addText(n.air);(n.patterns||[]).forEach(p=>addText(p.air))})}catch{}return [...vals].sort((a,b)=>a-b)}
function hdFCMutate(map,fleetId,fn){const s=hdFCState(map,fleetId);fn(s);hdFCSaveState(map,fleetId,s);hdFCRender()}
...
function hdFCReset(map,fleetId){const all=hdFCLoadAll();delete all[hdFCMapKey(map,fleetId)];hdFCSaveAll(all);hdFCRender()}
...
function hdFCHtml(map){const fleets=hdFCFleets(map),fleetId=hdFCSelection(map),s=hdFCState(map,fleetId),air=hdFCAirPower(s),airStatus=hdFCAirStatus(air,s.enemyAir),los=hdFCLOS(s),candidates=hdFCMapEnemyAirCandidates(map);return `<section id="hdFleetCalculator" class="hd-fc"><div class="hd-fc-head"><div><div class="eyebrow">FLEET CALCULATOR</div><h4>艦隊制空・索敵プランナー</h4></div><span>${hdFCEsc(map)}</span></div><p class="muted">艦載機の制空値と33式索敵スコアを同時計算。素索敵は装備を外した値＋艦固有の装備ボーナス分を入力してね。</p><div class="hd-fc-top"><label>保存編成<select id="hdFCFleetSelect"><option value="manual" ${fleetId==='manual'?'selected':''}>手動</option>${fleets.map(f=>`<option value="${hdFCEsc(f.id)}" ${fleetId===f.id?'selected':''}>${hdFCEsc(f.name)}</option>`).join('')}</select></label><label>司令部Lv<input type="number" min="1" max="120" value="${Number(s.hqLevel)||120}" data-hd-fc-hq></label><label>分岐点係数<input type="number" min="1" max="10" step="1" value="${Number(s.branchCoef)||1}" data-hd-fc-coef></label><label>分岐到達時の隻数<input type="number" min="1" max="7" value="${Number(s.shipCount)||6}" data-hd-fc-count></label></div>${fleetId!=='manual'?'<button type="button" class="ghost small" data-hd-fc-sync>保存編成の艦名を再読込</button>':''}<div class="hd-fc-grid"><article><div class="hd-fc-sub"><strong>艦娘の素索敵</strong><span>Σ√素索敵</span></div><div class="hd-fc-ships">${(s.ships||[]).slice(0,Math.max(6,Number(s.shipCount)||6)).map((x,i)=>`<label><span>${i+1}. ${hdFCEsc(x.name||'艦娘')}</span><input type="number" min="0" max="200" step="1" value="${Number(x.los)||0}" data-hd-fc-ship-los="${i}"></label>`).join('')}</div></article><article><div class="hd-fc-sub"><strong>装備</strong><button type="button" class="ghost small" data-hd-fc-add>＋装備</button></div><div class="hd-fc-gear">${(s.gear||[]).map((r,i)=>{const item=hdFCFind(r.name);return `<div class="hd-fc-gear-row"><select data-hd-fc-gear="${i}">${hdFCEquipOptions(r.name)}</select><label>搭載<input type="number" min="0" max="99" value="${Number(r.slot)||0}" data-hd-fc-slot="${i}"></label><label>★<input type="number" min="0" max="10" value="${Number(r.star)||0}" data-hd-fc-star="${i}"></label><label class="hd-fc-prof"><input type="checkbox" data-hd-fc-prof="${i}" ${r.maxProf?'checked':''}> &gt;&gt;</label><button type="button" class="ghost small" data-hd-fc-remove="${i}">×</button>${item?`<small>${hdFCEsc(item.category)} / 対空${Number(item.stats?.対空)||0} / 索敵${Number(item.stats?.索敵)||0}</small>`:''}</div>`}).join('')}</div></article></div><div class="hd-fc-results"><div><span>艦隊制空値</span><strong>${air}</strong><small>各スロット切り捨て合計</small></div><div class="${airStatus.cls}"><span>制空状態</span><strong>${hdFCEsc(airStatus.label)}</strong><small>${hdFCEsc(airStatus.detail)}</small></div><div><span>33式索敵</span><strong>${los.score.toFixed(2)}</strong><small>係数 ${los.coef}</small></div></div><div class="hd-fc-enemy"><label>敵制空値<input type="number" min="0" max="9999" value="${Number(s.enemyAir)||0}" data-hd-fc-enemy></label>${candidates.length?`<div class="hd-fc-chips"><span>海域データ候補</span>${candidates.map(v=>`<button type="button" class="ghost small" data-hd-fc-enemy-chip="${v}">${v}</button>`).join('')}</div>`:''}</div><details class="hd-fc-breakdown"><summary>索敵計算の内訳</summary><div>艦娘: ${los.shipLos.toFixed(3)}</div><div>装備係数前: ${los.equipRaw.toFixed(3)}</div><div>装備×分岐点係数: ${los.equip.toFixed(3)}</div><div>司令部補正: −${los.hq}</div><div>隻数補正: ${los.cq>=0?'+':''}${los.cq}</div></details><div class="hd-fc-mapnote"><b>この海域の索敵メモ</b><span>${hdFCEsc(hdFCMapNote(map))}</span></div><div class="hd-fc-actions"><button type="button" class="ghost small" data-hd-fc-reset>この編成の計算をリセット</button><a class="guide-link" href="https://wikiwiki.jp/kancolle/%E3%83%AB%E3%83%BC%E3%83%88%E5%88%86%E5%B2%90" target="_blank" rel="noopener">33式の仕様 ↗</a></div><p class="muted hd-fc-foot">※熟練度は「>>」の最大熟練を簡易反映。艦固有の装備ボーナスは装備側では自動判定しないため、その分は艦娘の素索敵入力へ足してね。</p></section>`}
function hdFCGearPane(){
 const fallback=document.getElementById('hdFallbackGearTools'),normal=document.querySelector('[data-map-pane="gear"]:not(#hdFallbackGearTools)');
