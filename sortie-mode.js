...
function hdSMNodeRows(map){
 ...
 return labels.sort((a,b)=>(Number(level[a]??999)-Number(level[b]??999))||String(a).localeCompare(String(b),undefined,{numeric:true}))
  .map(label=>{const base=hdSMNodeKind(graph,label);return {label,kind:hdSMEffectiveKind(map,label,base)}});
}
function hdSMNextNodeRows(map,draft){
 ...
 return labels.map(label=>{const base=hdSMNodeKind(graph,label);return {label,kind:hdSMEffectiveKind(map,label,base)}});
}
...
function hdSMEffectiveKind(map,label,baseKind,detail){
 const current=String(label||'').trim(),base=String(baseKind||hdSMNodeKind(hdSMGraph(map),current)||'normal');
 if(base!=='normal')return base;
 let row=detail;
 if(!row){try{if(typeof HD_NODE_DETAIL_OVERRIDES!=='undefined')row=HD_NODE_DETAIL_OVERRIDES?.[map]?.[current]||{}}catch{row={}}}
 const enemy=String(row?.enemy||'').trim();
 if(/^うずしお/.test(enemy))return 'vortex';
 if(/^能動分岐/.test(enemy)||/気のせいだった|敵影を見ず/.test(enemy))return 'safe';
 if(/^戦闘なし/.test(enemy)){
  if(/獲得|資源|アイテム|高速修復材|燃料|弾薬|鋼材|ボーキ/.test(enemy))return 'item';
  return 'safe';
 }
 return base;
}
...
function hdSMNodeIntel(map,row){
 row=row||{};const label=String(row.label||'').trim(),baseKind=String(row.kind||hdSMNodeKind(hdSMGraph(map),label)||'normal');
 let detail={};try{if(label&&typeof HD_NODE_DETAIL_OVERRIDES!=='undefined')detail=HD_NODE_DETAIL_OVERRIDES?.[map]?.[label]||{}}catch{}
 const kind=hdSMEffectiveKind(map,label,baseKind,detail);
...
function hdSMCurrentTacticHtml(map,draft){
 ...
 const graph=hdSMGraph(map),baseKind=hdSMNodeKind(graph,current),intel=hdSMNodeIntel(map,{label:current,kind:baseKind}),kind=intel.kind;
...
function hdSMRequiresAdvanceCheck(map,label){
 ...
 const kind=hdSMEffectiveKind(map,current,hdSMNodeKind(hdSMGraph(map),current));
...
function hdSMHudHtml(session,draft){
 const current=String(draft?.node||'').trim();if(!current)return '';
 const graph=hdSMGraph(session?.map),baseKind=hdSMNodeKind(graph,current),intel=hdSMNodeIntel(session?.map,{label:current,kind:baseKind}),kind=intel.kind,guard=hdSMAdvanceGuard(session,draft),branch=hdSMBranchHint(session?.map,draft);
...
window.hdSMEffectiveKind=hdSMEffectiveKind;
