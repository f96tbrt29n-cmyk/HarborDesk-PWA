...
function hdSMCanReachBoss(map,label){
 const graph=hdSMGraph(map),from=String(label||'').trim(),target=String(graph?.boss||graph?.goal||'').trim();
 if(!graph||!Array.isArray(graph.edges)||!from||!target)return null;
...
function hdSMNextNodeButtonHtml(map,row,locked=false){
 const intel=hdSMNodeIntel(map,row),reachable=hdSMCanReachBoss(map,intel.label),distance=reachable===true?hdSMBossDistance(map,intel.label):null,battles=reachable===true?hdSMBossBattleDistance(map,intel.label):null;
 const routeLabel=reachable===false?'構造図上 逸れ候補｜ボス接続なし':reachable!==true?'構造図上 経路不明':distance===0?'構造図上 ボス到達':'構造図上 ボス接続｜選択後 あと'+distance+'マス・'+battles+'戦';
...
function hdSMHudHtml(session,draft){
 ...
 const next=guard.confirmed&&!guard.retreat?hdSMNextNodeRows(session?.map,draft):[],battleCount=hdSMBattleCount(session?.map,draft?.routeNodes||[]),bossDistance=hdSMBossDistance(session?.map,current),bossBattles=hdSMBossBattleDistance(session?.map,current);
 const nextHtml=next.length?'<div class="hd-sm-hud-next"><span>NEXT</span><div>'+next.slice(0,3).map(x=>{const ni=hdSMNodeIntel(session?.map,x),reachable=hdSMCanReachBoss(session?.map,x.label),distance=reachable===true?hdSMBossDistance(session?.map,x.label):null,battles=reachable===true?hdSMBossBattleDistance(session?.map,x.label):null,routeText=reachable===false?'逸れ候補・接続なし':reachable!==true?'経路不明':distance===0?'ボス到達':'ボス接続・あと'+distance+'マス/'+battles+'戦';return '<button type="button" class="'+(reachable===false?'route-off':reachable===true?'route-on':'')+'" data-hd-sm-hud-node="'+hdSMEsc(x.label)+'"><b>'+hdSMEsc(x.label)+'</b><small>'+hdSMEsc(hdSMNodeKindLabel(ni.kind))+'・'+hdSMEsc(ni.formation)+'</small><em>'+hdSMEsc(routeText)+'</em></button>'}).join('')+'</div>'+(next.length>3?'<em>+'+(next.length-3)+'</em>':'')+'</div>':'';
 ...
 const progressHtml='<div class="hd-sm-hud-progress"><span>戦闘 <b>'+battleCount+'</b></span><span>'+(bossDistance===0?'ボス到達':bossDistance==null?'ボス距離 —':'構造図最短 ボスまで <b>'+bossDistance+'マス</b>')+'</span><span>'+(bossBattles==null?'最少戦闘 —':bossBattles===0?'最少戦闘あと 0':'最少戦闘あと <b>'+bossBattles+'</b>')+'</span></div>';
