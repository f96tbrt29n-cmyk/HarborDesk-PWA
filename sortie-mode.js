...
function hdSMObjectiveRouteState(map,draft){
 const current=String(draft?.node||'').trim(),target=hdSMSelectedObjective(map,draft);
 if(!current||!target)return {active:false,current,target,alternatives:[]};
 const reachable=hdSMCanReachBoss(map,current,target);
 if(reachable!==false)return {active:false,current,target,reachable,alternatives:[]};
 const alternatives=hdSMObjectiveOptions(map).filter(x=>x.label!==target&&hdSMCanReachBoss(map,current,x.label)===true);
 return {active:true,current,target,targetName:hdSMRouteTargetName(map,target),alternatives};
}
...
function hdSMNextNodeButtonHtml(map,row,locked=false,objectiveTarget='',preferredLabel=''){
 const intel=hdSMNodeIntel(map,row),reachable=hdSMCanReachBoss(map,intel.label,objectiveTarget),targetName=hdSMRouteTargetName(map,objectiveTarget),preferred=String(preferredLabel||'')===intel.label,remainSteps=reachable===false?null:hdSMBossDistance(map,intel.label,objectiveTarget),remainBattles=reachable===false?null:hdSMBossBattleDistance(map,intel.label,objectiveTarget),routeLabel=(reachable===true?'構造図上 '+targetName+'接続':reachable===false?'構造図上 逸れ候補':'構造図上 経路不明')+(preferred?' ・ 最短経路':'');
 const progressLabel=reachable===false?'':remainSteps===0?'目標到達':remainSteps==null?'残り距離 —':'目標まで '+remainSteps+'マス / 最少'+(remainBattles==null?'—':remainBattles)+'戦';
 const detail=[intel.enemy?('敵 '+intel.enemy):'',intel.air?('制空 '+intel.air):''].filter(Boolean).join(' / ')||intel.summary;
 return '<button type="button" class="hd-sm-next '+hdSMEsc(intel.kind)+(preferred?' shortest':'')+(locked?' locked':'')+'" data-hd-sm-next-node="'+hdSMEsc(intel.label)+'"'+(locked?' disabled aria-disabled="true"':'')+'><b>'+hdSMEsc(intel.label)+'</b><small>'+hdSMEsc(hdSMNodeKindLabel(intel.kind))+'</small><i>'+(locked?'確認待ち':'次へ')+'</i><span class="hd-sm-next-risk">'+hdSMEsc(intel.badge)+'</span><span class="hd-sm-next-formation">基本陣形 '+hdSMEsc(intel.formation)+'</span><span class="hd-sm-next-route '+(reachable===false?'off':reachable===true?'on':'unknown')+'">'+routeLabel+'</span>'+(progressLabel?'<span class="hd-sm-next-progress">'+hdSMEsc(progressLabel)+'</span>':'')+'<span class="hd-sm-next-caution">'+hdSMEsc(intel.caution)+'</span><em>'+hdSMEsc(detail)+'</em></button>';
}
...
function hdSMHudHtml(session,draft){
 ...
 const nextHtml=next.length?'<div class="hd-sm-hud-next"><span>NEXT</span><div>'+next.slice(0,3).map(x=>{const ni=hdSMNodeIntel(session?.map,x),reachable=hdSMCanReachBoss(session?.map,x.label,objectiveTarget),targetName=hdSMRouteTargetName(session?.map,objectiveTarget),preferred=remainingPath[1]===x.label,remainSteps=reachable===false?null:hdSMBossDistance(session?.map,x.label,objectiveTarget),remainBattles=reachable===false?null:hdSMBossBattleDistance(session?.map,x.label,objectiveTarget),progress=remainSteps===0?'到達':remainSteps==null?'':'残'+remainSteps+'マス/'+(remainBattles==null?'—':remainBattles)+'戦';return '<button type="button" class="'+(reachable===false?'route-off':reachable===true?'route-on':'')+(preferred?' shortest':'')+'" data-hd-sm-hud-node="'+hdSMEsc(x.label)+'"><b>'+hdSMEsc(x.label)+'</b><small>'+hdSMEsc(hdSMNodeKindLabel(ni.kind))+'・'+hdSMEsc(ni.formation)+'</small><em>'+(reachable===true?targetName+'接続':reachable===false?'逸れ候補':'経路不明')+(preferred?'・最短':'')+(progress?'・'+progress:'')+'</em></button>'}).join('')+'</div>'+(next.length>3?'<em>+'+(next.length-3)+'</em>':'')+'</div>':'';
...
window.hdSMObjectiveRouteState=hdSMObjectiveRouteState;
window.hdSMObjectiveRouteWarningHtml=hdSMObjectiveRouteWarningHtml;
