...
function hdSMNextNodeButtonHtml(map,row,locked=false,objectiveTarget='',preferredLabel=''){
 const intel=hdSMNodeIntel(map,row),reachable=hdSMCanReachBoss(map,intel.label,objectiveTarget),targetName=hdSMRouteTargetName(map,objectiveTarget),preferred=String(preferredLabel||'')===intel.label,routeLabel=(reachable===true?'構造図上 '+targetName+'接続':reachable===false?'構造図上 逸れ候補':'構造図上 経路不明')+(preferred?' ・ 最短経路':'');
...
function hdSMHudHtml(session,draft){
 ...
 const nextHtml=next.length?'<div class="hd-sm-hud-next"><span>NEXT</span><div>'+next.slice(0,3).map(x=>{const ni=hdSMNodeIntel(session?.map,x),reachable=hdSMCanReachBoss(session?.map,x.label,objectiveTarget),targetName=hdSMRouteTargetName(session?.map,objectiveTarget),preferred=remainingPath[1]===x.label;return '<button type="button" class="'+(reachable===false?'route-off':reachable===true?'route-on':'')+(preferred?' shortest':'')+'" data-hd-sm-hud-node="'+hdSMEsc(x.label)+'"><b>'+hdSMEsc(x.label)+'</b><small>'+hdSMEsc(hdSMNodeKindLabel(ni.kind))+'・'+hdSMEsc(ni.formation)+'</small><em>'+(reachable===true?targetName+'接続':reachable===false?'逸れ候補':'経路不明')+(preferred?'・最短':'')+'</em></button>'}).join('')+'</div>'+(next.length>3?'<em>+'+(next.length-3)+'</em>':'')+'</div>':'';
