...
function buildChangeSummary(oldSnap,next){
 ...
 const shipsDiff=diffKeys(oldSnap.ships,next.ships),equipmentDiff=diffKeys(oldSnap.equipment,next.equipment);
...
const semanticallyCurrent=oldSnapshot&&oldSnapshot.source?.commit===snapshot.source?.commit&&oldSnapshot.clientRules?.source?.commit===snapshot.clientRules?.source?.commit&&stable(oldSnapshot.ships)===stable(snapshot.ships)&&stable(oldSnapshot.equipment)===stable(snapshot.equipment)&&stable(oldSnapshot.exslotBaseTypeIds)===stable(snapshot.exslotBaseTypeIds)&&stable(oldSnapshot.exslotGlobalItemIds)===stable(snapshot.exslotGlobalItemIds)&&stable(oldSnapshot.exslotItemRules)===stable(snapshot.exslotItemRules)&&stable(oldSnapshot.exslotLimitTypeIds)===stable(snapshot.exslotLimitTypeIds)&&stable(oldSnapshot.clientRules?.slotExclusions||[])===stable(snapshot.clientRules?.slotExclusions||[])&&stable(oldSnapshot.clientRules?.equipTypeSpOverrides||{})===stable(snapshot.clientRules?.equipTypeSpOverrides||{})&&stable(oldSnapshot.clientRules?.pickerTypeOverrides||{})===stable(snapshot.clientRules?.pickerTypeOverrides||{});
