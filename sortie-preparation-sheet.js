...
function hdSPSAutoHtml(auto){
 if(!auto)return '';
 const label=typeof hdFEAutoStatusLabel==='function'?hdFEAutoStatusLabel(auto.status):auto.status==='ready'?'OK':auto.status==='missing'?'不足/不可':'要確認',cls=auto.status==='ready'?'ok':auto.status==='missing'?'bad':'warn',gameDiff=typeof hdFEGameMatchHtml==='function'?hdFEGameMatchHtml(auto.gameMatch):'';
 return `<section class="hd-sps-card hd-sps-auto"><div class="hd-sps-card-head"><div><span>自動判定</span><strong>同期鮮度・艦状態・補給・ゲーム反映・編成・装備・制空・索敵</strong></div><b class="${cls}">${hdSPSEsc(label)}</b></div><div class="hd-sps-auto-grid">${(auto.checks||[]).map(x=>`<div class="${x.status}"><span>${hdSPSEsc(x.label)}</span><b>${hdSPSEsc(typeof hdFEAutoStatusLabel==='function'?hdFEAutoStatusLabel(x.status):x.status)}</b><small>${hdSPSEsc(x.detail||'')}</small></div>`).join('')}</div>${gameDiff}${auto.scouting?.available&&auto.scouting?.checks?.length?`<div class="hd-sps-auto-los">${auto.scouting.checks.map(x=>`<span class="${x.status}">${hdSPSEsc(x.label)}：推定33式 ${Number(auto.scouting.score).toFixed(2)} / 安全域 ${x.safe}</span>`).join('')}</div>`:''}<p class="muted">「要確認」はデータ不足・同期が古い・海域条件が分岐ごとに異なる項目。出撃前にゲーム側で最終確認してね。</p></section>`;
}
