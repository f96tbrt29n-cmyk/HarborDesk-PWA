...
function hdInteractiveMap(){
  return typeof selectedMap!=='undefined'&&selectedMap?selectedMap:null;
}
function hdInteractiveGraph(map){return typeof HD_MAP_GRAPHS!=='undefined'?HD_MAP_GRAPHS[map]:null}
function hdInteractiveKind(map,label){
  const g=hdInteractiveGraph(map);return g&&typeof hdMapKind==='function'?hdMapKind(g,label):'normal';
}
function hdNodeLabel(group){
...
function hdShowNodeInfo(map,label){
  ...
  const kind=hdInteractiveKind(map,label);
  const info=HD_MAP_KIND_INFO[kind]||HD_MAP_KIND_INFO.normal;
...
function hdClearRouteHighlight(){
  document.querySelectorAll('.hd-map-route').forEach(p=>p.classList.remove('highlight'));
}
function hdHighlightShortest(map){
  hdClearRouteHighlight();
  const path=hdShortestPath(map);if(path.length<2)return;
  const pane=document.querySelector('[data-map-pane="map"]');
  const paths=[...(pane?.querySelectorAll('.hd-map-route')||[])];
  const g=hdInteractiveGraph(map);if(!g)return;
  const wanted=new Set(path.slice(0,-1).map((a,i)=>`${a}>${path[i+1]}`));
  paths.forEach((el,i)=>{const e=g.edges[i];if(e&&wanted.has(`${e[0]}>${e[1]}`))el.classList.add('highlight')});
...
document.addEventListener('click',e=>{
  const group=e.target.closest?.('.hd-map-node');
  if(group){
