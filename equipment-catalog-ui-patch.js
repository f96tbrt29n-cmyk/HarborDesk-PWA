...
hdRenderEquipmentCatalog=function(){
 ...
 list.innerHTML=rows.map(x=>`<article class="hd-equip-ref-card"><div class="hd-equip-ref-head"><div><strong>${hdEsc(x.name)}</strong><div class="muted">${hdEsc(x.category)}</div></div><button class="primary small" type="button" data-hd-equip-add="${hdEsc(x.name)}">台帳へ追加</button></div><div class="hd-equip-stats">${hdEquipStatText(x).map(s=>`<span>${hdEsc(s)}</span>`).join('')||'<span>特殊効果装備</span>'}</div><div class="hd-equip-tags">${(x.tags||[]).map(t=>`<span>${hdEsc(t)}</span>`).join('')}</div><p>${hdEsc(x.role||'')}</p><div class="hd-equip-ref-grid"><div><span>改修</span><strong>${hdEsc(x.improve||'未確認')}</strong></div><div><span>入手</span><strong>${hdEsc(x.obtain||'未確認')}</strong></div>${x.equip?`<div class="wide"><span>主な搭載</span><strong>${hdEsc(x.equip)}</strong></div>`:''}${x.special?`<div class="wide"><span>特殊効果・注意</span><strong>${hdEsc(x.special)}</strong></div>`:''}<div class="wide"><span>更新・補足</span><strong>${hdEsc(x.update||'なし')}</strong></div></div><a class="guide-link" href="${hdEquipWikiUrl(x.name)}" target="_blank" rel="noopener">攻略Wikiで詳細 ↗</a></article>`).join('')||'<div class="empty">条件に合う装備がないよ</div>';
};
document.addEventListener('click',e=>{
 const a=e.target.closest?.('[data-hd-equip-add]');if(!a)return;
 const item=HD_EQUIPMENT_CATALOG.find(x=>x.name===a.dataset.hdEquipAdd);if(!item||typeof openEquipment!=='function')return;
 e.preventDefault();e.stopImmediatePropagation();
 openEquipment({name:item.name,category:item.category,count:1,star:0,targetStar:String(item.improve||'').includes('可')?10:0,assigned:'',memo:[`用途: ${item.role||''}`,item.equip?`主な搭載: ${item.equip}`:'',item.special?`特殊効果・注意: ${item.special}`:'',`入手: ${item.obtain||''}`,item.update||''].filter(Boolean).join('\n')});
},true);
