...
function hdQNSavePins(v){localStorage.setItem(HD_QN_PIN_KEY,JSON.stringify(v));window.dispatchEvent(new CustomEvent('hd:quick-nav-updated'))}
function hdQNLoadRecent(){try{return JSON.parse(localStorage.getItem(HD_QN_RECENT_KEY)||'[]')||[]}catch{return []}}
function hdQNSaveRecent(v){localStorage.setItem(HD_QN_RECENT_KEY,JSON.stringify(v));window.dispatchEvent(new CustomEvent('hd:quick-nav-updated'))}
function hdQNRecordRecent(id){const rows=hdQNLoadRecent().filter(x=>x.id!==id);rows.unshift({id,at:Date.now()});hdQNSaveRecent(rows.slice(0,8))}
...
function hdQNRenderList(filter=''){
  ...
  const q=String(filter||'').trim().toLowerCase(),pins=hdQNLoadPins(),pinSet=new Set(pins),recent=new Map(hdQNLoadRecent().map((x,i)=>[x.id,i])),rows=hdQNSections().filter(x=>!q||x.title.toLowerCase().includes(q)||x.id.toLowerCase().includes(q));
  rows.sort((a,b)=>{const pa=pinSet.has(a.id),pb=pinSet.has(b.id);if(pa!==pb)return pb-pa;const ra=recent.has(a.id)?recent.get(a.id):999,rb=recent.has(b.id)?recent.get(b.id):999;if(ra!==rb)return ra-rb;return a.title.localeCompare(b.title,'ja')});
