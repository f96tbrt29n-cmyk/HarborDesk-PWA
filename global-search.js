...
const HD_GS_VIEW_KEY='harbordesk-session-global-search-view-v1';
...
function hdGSViewLoad(){try{return JSON.parse(sessionStorage.getItem(HD_GS_VIEW_KEY)||'{}')||{}}catch{return {}}}
function hdGSViewSave(patch={}){const next={...hdGSViewLoad(),...patch};try{sessionStorage.setItem(HD_GS_VIEW_KEY,JSON.stringify(next))}catch{}return next}
function hdGSResetFilters(){hdGSCategory='all';hdGSOwnedOnly=false;hdGSViewSave({category:'all',ownedOnly:false});document.querySelectorAll('[data-hd-gs-cat]').forEach(b=>b.classList.toggle('active',b.dataset.hdGsCat==='all'));hdGSRender()}
...
function hdGSOpen(query=''){hdGSEnsure();if(typeof hdQNClose==='function')hdQNClose();const d=document.getElementById('hdGlobalSearchDialog'),i=document.getElementById('hdGSSearch'),view=hdGSViewLoad(),valid=['all','map','ship','equipment','quest','expedition','feature'];if(i)i.value=query;hdGSCategory=valid.includes(view.category)?view.category:'all';hdGSOwnedOnly=!!view.ownedOnly;document.querySelectorAll('[data-hd-gs-cat]').forEach(b=>b.classList.toggle('active',b.dataset.hdGsCat===hdGSCategory));hdGSRender();if(typeof d.showModal==='function'){if(!d.open)d.showModal()}else d.setAttribute('open','');window.dispatchEvent(new CustomEvent('hd:global-search-open'));setTimeout(()=>i?.focus(),50)}
function hdGSClose(){hdGSNavSeq++;const d=document.getElementById('hdGlobalSearchDialog');if(!d)return;if(typeof d.close==='function'&&d.open)d.close();else d.removeAttribute('open');window.dispatchEvent(new CustomEvent('hd:global-search-close'))}
