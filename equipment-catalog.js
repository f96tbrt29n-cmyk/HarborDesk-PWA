...
function hdEnsureEquipmentCatalog(){
 ...
 list?.classList.toggle('hd-compact',compact);if(compactBtn)compactBtn.textContent=compact?'詳細表示':'コンパクト';const hint=document.getElementById('hdEquipCatalogCompactHint');if(hint)hint.hidden=!compact;
...
function hdRenderEquipmentCatalog(){
 ...
 const dirty=!!q||hdEquipCatalogFilter!=='すべて';
 ...
 document.getElementById('hdEquipmentCatalog')?.querySelectorAll('[data-hd-equip-reset]').forEach(reset=>{reset.disabled=!dirty;reset.classList.toggle('is-active',dirty)})
