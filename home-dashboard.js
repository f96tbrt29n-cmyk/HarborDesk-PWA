...
function homeFleetIssueRows(ships=[]){
 return (Array.isArray(ships)?ships:[]).map((s,index)=>{
  const max=Number(s?.maxHp)||0,now=Number(s?.nowHp)||0,ratio=max>0?now/max:1,cond=homeFleetCondMeta(s?.cond),parts=[];
  let cls='';
  if(max>0&&ratio<=.25){parts.push('HP '+now+'/'+max);cls='danger'}
  else if(max>0&&ratio<=.5){parts.push('HP '+now+'/'+max);cls='warn'}
  if(cond.cls==='fatigue'){parts.push('疲労 cond '+cond.value);if(!cls)cls='warn'}
  if(!parts.length)return null;
  return {name:String(s?.name||('第'+(index+1)+'艦')),label:parts.join('・'),cls};
 }).filter(Boolean);
}
...
function homeGuideState(){
 try{
  const data=JSON.parse(localStorage.getItem(HD_HOME_GUIDE_KEY)||'{}');
  const custom=Array.isArray(data.custom)?data.custom.filter(x=>x&&typeof x.id==='string'&&typeof x.title==='string'&&HD_HOME_GUIDE_GROUPS.some(g=>g.id===x.category)):[];
  const done=Array.isArray(data.done)?data.done.filter(x=>typeof x==='string'):[];
  const legacy=new Set(),shared=new Set();
  for(const task of custom){
   if(!['oneTimeQuest','baseSortie','basePrep'].includes(task.source))continue;
   const old=homeGuideItemKey(task.category,task.id,task.map,task.scope);
   if(done.includes(old)){legacy.add(old);shared.add(homeGuideGoalKey(task))}
  }
...
function homeGuideGoalKey(task,now=Date.now()){
 if(['oneTimeQuest','baseSortie','basePrep'].includes(task.source))return `shared:${task.source}:${task.ref}`;
 const key=homeGuideItemKey(task.category,task.id,task.map,task.scope);
...
function homeGuideRender(){
 ...
 const completed=HD_HOME_GUIDE_GROUPS.reduce((n,g)=>n+g.steps.filter(([id])=>done.has(homeGuideItemKey(g.id,id,map,g.id==='map'?'map':'global'))).length,0)+state.custom.filter(x=>(x.scope!=='map'||x.map===map)&&goalDone(x,homeGuideGoalKey(x))).length;
...
function renderHomeDashboard(){
 ...
 if(syncHost){
  syncHost.className='home-sync-card '+syncInfo.state;
   const syncNote=syncInfo.state==='drift'
    ?`<div class="home-sync-note danger"><b>台帳に差異あり。</b> 艦娘 ${Number(syncInfo.audit?.ships?.current)||0}/${Number(syncInfo.audit?.ships?.source)||0}隻・装備 ${Number(syncInfo.audit?.equipment?.current)||0}/${Number(syncInfo.audit?.equipment?.source)||0}個。ゲーム同期をもう一度実行して台帳を戻そう。</div>`
    :syncInfo.state==='partial'
 ...
 if(nextHost){
  const urgentTimer=nextTimer&&nextMs<=60*60*1000;
   const syncNeedsAttention=['missing','partial','warn','drift'].includes(syncInfo.state);
  ...
  if(urgentTimer){
   ...
   nextHost.innerHTML=`<button type="button" class="home-next-main" data-home-jump="expeditions"><span>もうすぐ終わる</span><strong>${homeEsc(nextTimer.kind)}・${homeEsc(nextTimer.name||'タイマー')}</strong><small>あと ${homeEsc(timeText)}</small></button><button type="button" class="ghost small" data-home-jump="expeditions">タイマーを見る</button>`;
    ...
    const missingText=syncInfo.state==='drift'
     ?`台帳差異: 艦娘 ${Number(syncInfo.audit?.ships?.current)||0}/${Number(syncInfo.audit?.ships?.source)||0}隻・装備 ${Number(syncInfo.audit?.equipment?.current)||0}/${Number(syncInfo.audit?.equipment?.source)||0}個`
     :syncInfo.state==='partial'&&syncInfo.missing.length?`次に開く: ${homeSyncMissingGuide(syncInfo.missing).join(' → ')||'母港'}`:'艦娘・装備・資源を最新状態にしよう';
    const syncAction=syncInfo.state==='drift'?'台帳を再同期する':syncInfo.state==='partial'?'同期を補完する':'先に更新しておく';
    nextHost.innerHTML=`<button type="button" class="home-next-main" data-home-jump="kancolleImport"><span>${homeEsc(syncAction)}</span><strong>ゲーム同期 ${homeEsc(syncInfo.label)}</strong><small>${homeEsc(missingText)}</small></button><a class="ghost small home-next-game" href="https://play.games.dmm.com/game/kancolle">艦これを開く</a>`;
 ...
 if(fleetHost){
  if(!fleetRows.length){
   ...
   const conds=ships.map(s=>homeFleetCondMeta(s.cond)),kira=conds.filter(x=>x.cls==='kira').length,fatigue=conds.filter(x=>x.cls==='fatigue').length,issueRows=homeFleetIssueRows(ships);
