...
function hdSLRecordEntry(input={}){
 ...
 for(const k of ['sessionId','fleetId','fleetName','strategy','strategyLabel','startedAt','durationMs','fleetSnapshot','readinessSnapshot','repeatIndex','repeatFromSessionId','chainRootSessionId','source','gameSortieKey','gameNodeNo','gameNodeLabel','gameBossCellNo','gameBossCellLabel','gameRouteNodes','gameRouteLabels','gameBattleResults']){
...
function hdSLFiltered(){
 const rows=hdSLLoad();
 if(hdSLFilter==='all')return rows;
 if(hdSLFilter==='today'){const d=new Date(),start=new Date(d.getFullYear(),d.getMonth(),d.getDate()).getTime();return rows.filter(x=>x.at>=start)}
 if(hdSLFilter==='hunt')return rows.filter(x=>x.huntId);
 if(hdSLFilter.startsWith('objective:')){
  const parts=hdSLFilter.split(':'),map=parts[1]||'',target=parts.slice(2).join(':');
  return rows.filter(x=>String(x.map||'')===map&&String(x.objectiveTarget||'')===target);
 }
 return rows.filter(x=>x.map===hdSLFilter);
}
...
function hdSLRender(){
 const host=document.getElementById('sortieLog')||document.getElementById('hdSortieLog');if(!host)return;const rows=hdSLFiltered(),st=hdSLStats(rows),all=hdSLLoad(),hunts=hdSLHunts().filter(x=>!x.obtained),maps=[...new Set(all.map(x=>x.map).filter(Boolean))].sort((a,b)=>a.localeCompare(b,undefined,{numeric:true})),objectives=[...new Map(all.filter(x=>String(x.objectiveTarget||'').trim()).map(x=>{const map=String(x.map||''),target=String(x.objectiveTarget||'').trim();return [map+'|'+target,{map,target}]})).values()].sort((a,b)=>a.map.localeCompare(b.map,undefined,{numeric:true})||a.target.localeCompare(b.target,undefined,{numeric:true}));
 ...
 const filters=document.getElementById('hdSLFilters');if(filters)filters.innerHTML=[['all','全期間'],['today','今日'],['hunt','掘りのみ'],...maps.map(m=>[m,m]),...objectives.map(x=>['objective:'+x.map+':'+x.target,x.map+' '+x.target])].map(([id,label])=>`<button type="button" class="ghost small${hdSLFilter===id?' active':''}" data-hd-sl-filter="${hdSLEsc(id)}">${hdSLEsc(label)}</button>`).join('');
 ...
 const list=document.getElementById('hdSLList');if(list)list.innerHTML=rows.slice(0,30).map(x=>`<article class="hd-sl-row"><div class="hd-sl-main"><div><strong>${hdSLEsc(x.map)} ${hdSLEsc(x.node||'')}</strong><span class="hd-sl-result ${x.result==='S'?'s':x.result==='撤退'?'retreat':''}">${hdSLEsc(x.result)}</span>${x.boss?'<span class="hd-sl-badge">ボス</span>':''}${x.objectiveTarget?`<span class="hd-sl-badge objective">目標 ${hdSLEsc(x.objectiveTarget)}</span>`:''}${x.huntShip?`<span class="hd-sl-badge hunt">掘り ${hdSLEsc(x.huntShip)}</span>`:''}</div><small>${hdSLFmtAt(x.at)}${x.drop?`・Drop ${hdSLEsc(x.drop)}`:''}${x.buckets?`・バケツ${x.buckets}`:''}</small></div><div class="hd-sl-cost">${(x.fuel||x.ammo||x.steel||x.bauxite)?`燃${x.fuel||0} 弾${x.ammo||0} 鋼${x.steel||0} ボ${x.bauxite||0}`:'資源未記録'}</div>${x.memo?`<p>${hdSLEsc(x.memo)}</p>`:''}<button type="button" class="ghost small" data-hd-sl-delete="${x.id}">削除・連動を戻す</button></article>`).join('')||'<div class="empty">まだ出撃ログはないよ。上のフォームから1周ずつ記録できる。</div>';hdSLRenderTargetHint();
}
function hdSLEnsure(){
