...
function hdSortieMinePane(){
 const fallback=document.getElementById('hdFallbackMineTools'),normal=document.querySelector('.map-tabs-shell [data-map-pane="mine"]');
...
function hdRenderSortieReadiness(){
 ...
 const pane=hdSortieMinePane();if(!pane)return;
 const html=hdSortieHtml(selectedMap),template=document.createElement('template');template.innerHTML=html.trim();const next=template.content.firstElementChild;if(!next)return;
 const old=pane.querySelector('#hdSortieReadiness');if(old?.parentNode)old.parentNode.replaceChild(next,old);else pane.appendChild(next);
 const sel=pane.querySelector('#hdSortieFleetSelect');if(sel)sel.addEventListener('change',e=>hdSortieSetSelection(selectedMap,e.target.value));
}
...
window.addEventListener('hd:map-rendered',e=>{const d=e?.detail||{};if(d.mode==='empty'||!d.map)return;setTimeout(hdRenderSortieReadiness,0)});
