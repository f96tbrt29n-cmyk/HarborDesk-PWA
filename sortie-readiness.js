...
function hdSortieHtml(map){
 ...
 const fleetId=hdSortieSelection(map),fleet=fleets.find(x=>x.id===fleetId)||fleets[0],autoInfo=hdSortieAutoChecks(map,fleet),manual=hdSortieManualChecks(map,autoInfo.adv),state=hdSortieState(map,fleet.id),sum=hdSortieSummary(manual,state,autoInfo.checks),sortedChecks=hdSortieSortedChecks(autoInfo.checks),problemChecks=sortedChecks.filter(x=>x.state!=='ok'),nextCheck=problemChecks[0]||null,remainingProblems=problemChecks.slice(1),okChecks=sortedChecks.filter(x=>x.state==='ok'),autoState=sum.autoWarn?'warn':sum.autoNote?'note':'ok',autoLabel=sum.autoWarn?`要確認 ${sum.autoWarn}件`:sum.autoNote?`注意 ${sum.autoNote}件`:'自動確認OK';
