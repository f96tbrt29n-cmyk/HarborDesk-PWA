...
async function hdDXCollect(forceFresh=false){
  const local=hdDXLocalHealth(),modules=hdDXModuleHealth(),identity=hdDXShipIdentityHealth(),expected=`harbordesk-pwa-v${typeof HD_APP_BUILD!=='undefined'?HD_APP_BUILD:'?'}`,master=(typeof HD_SHIP_MASTER_AUDIT!=='undefined'?HD_SHIP_MASTER_AUDIT:window.HD_SHIP_MASTER_AUDIT)||null;
  ...
  return {at:Date.now(),master,masterSource,pickerSource,masterChanges,masterDate,pickerDate,upstream,online:navigator.onLine,standalone:!!(window.matchMedia?.('(display-mode: standalone)').matches||navigator.standalone),local,modules,identity,expected,swSupported,swControlled,swRegistered,swActive,cacheNames,currentCache,criticalCached,usage,quota,persisted,snapshots,backupDays,status,issues};
}
...
document.addEventListener('click',e=>{if(e.target.closest?.('[data-dx-recheck]')){hdDXRender(true);return}if(e.target.closest?.('[data-dx-ship-identity]')){hdDXRepairShipIdentity();hdDXRender();return}if(e.target.closest?.('[data-dx-reinit]')){hdDXReinit();return}if(e.target.closest?.('[data-dx-cache]')){hdDXCacheRepair();return}if(e.target.closest?.('[data-dx-update]')){if(typeof hdCheckForUpdate==='function')hdCheckForUpdate(true);return}if(e.target.closest?.('[data-dx-copy]')){hdDXCopy();return}});
