...
async function hdDXCollect(){
  ...
  return {at:Date.now(),master,masterSource,pickerSource,masterDate,pickerDate,online:navigator.onLine,standalone:!!(window.matchMedia?.('(display-mode: standalone)').matches||navigator.standalone),local,modules,expected,swSupported,swControlled,swRegistered,swActive,cacheNames,currentCache,criticalCached,usage,quota,persisted,snapshots,backupDays,status,issues};
}
