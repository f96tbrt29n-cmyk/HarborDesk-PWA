...
async function hdDXCollect(forceFresh=false){
  ...
  const restoreSafety={
    ...
    snapshotApi:typeof hdPHCreateSnapshot==='function'&&typeof hdPHGetSnapshots==='function'&&typeof hdPHOpenDb==='function',
    storeReady:false,
    busy:!!window.__hdBackupRestoreBusy,
    error:''
  };
  if(restoreSafety.indexedDB&&restoreSafety.snapshotApi){
    try{const db=await hdPHOpenDb();restoreSafety.storeReady=!!db;try{db?.close?.()}catch{}}
    catch(err){restoreSafety.error=String(err?.message||err||'IndexedDB open failed')}
  }
  restoreSafety.ready=restoreSafety.indexedDB&&restoreSafety.snapshotApi&&restoreSafety.storeReady;
