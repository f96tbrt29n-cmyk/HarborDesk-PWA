...
async function hdDXCollect(forceFresh=false){
  ...
  return {at:Date.now(),master,masterSource,pickerSource,masterChanges,masterDate,pickerDate,upstream,online:navigator.onLine,standalone:!!(window.matchMedia?.('(display-mode: standalone)').matches||navigator.standalone),local,modules,identity,imageIntegrity,expected,swSupported,swControlled,swRegistered,swActive,cacheNames,currentCache,criticalCached,usage,quota,persisted,snapshots,backupDays,status,issues};
}
