...
function hdEAcandidates(group,limit=5){
 ...
 const owned=hdEAownedMap(),matches=cat.filter(x=>hdEAmatches(x,group));
 const ownedMatches=matches.filter(x=>(owned.get(hdEAnormalize(x.name))?.count||0)>0).sort((a,b)=>hdEAitemScore(b)-hdEAitemScore(a));
 const unowned=matches.filter(x=>(owned.get(hdEAnormalize(x.name))?.count||0)<=0).sort((a,b)=>hdEAitemScore(b)-hdEAitemScore(a));
 const keep=Math.max(limit,ownedMatches.length);
 return [...ownedMatches,...unowned].slice(0,keep);
}
