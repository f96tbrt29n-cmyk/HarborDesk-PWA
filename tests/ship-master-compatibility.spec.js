...
test('ship identity diagnostics detects and repairs master ID drift', async ({ page }) => {
  ...
  await page.waitForFunction(() =>
    typeof window.hdDXShipIdentityHealth === 'function' &&
    typeof window.hdDXRepairShipIdentity === 'function'
  );
...
test('HarborDesk Userscript is installable and page-context ready', async ({ page }) => {
  ...
  expect(data.usesPopup).toBe(true);
...
test('roster sort and compact mode persist in session', async ({ page }) => {
  ...
  await page.addInitScript(() => {
    ...
    localStorage.setItem('harbordesk-ship-roster-v1', JSON.stringify([
      {id:'1',name:'しょうかく',level:99,tags:[]},
      {id:'2',name:'あかぎ',level:90,tags:[]},
      {id:'3',name:'かが',level:94,tags:[]}
    ]));
  });
  await boot(page,errors);
  const data=await page.evaluate(()=>({
...
test('mobile workspace uses compact section picker', async ({ page }) => {
  ...
  expect(['flex','grid']).toContain(data.display);
...
test('reset actions enable only when filters are active', async ({ page }) => {
  ...
  const initial=await page.evaluate(()=>{
    window.renderShipRoster?.();window.hdEnsureShipDatabase?.();window.hdEnsureEquipmentCatalog?.();window.hdRenderEquipmentCatalog?.();
    return {
...
test('global search is one tap from header menu and shows destination hints', async ({ page }) => {
  const errors=[];
  await page.addInitScript(() => {
  ...
  const data=await page.evaluate(()=>{
    ...
    const menu=document.querySelector('.hd-header-more [data-hd-gs-open]');
    ...
    return {header:!!header,menu:!!menu,menuText:menu?.textContent||'',dest:first?.querySelector('.hd-gs-dest')?.textContent||'',owned:first?.classList.contains('owned')||false};
  });
...
test('core quest and timer filters keep completed items out of the way', async ({ page }) => {
  ...
  const data=await page.evaluate(()=>{
    ...
    if(typeof coreListFilterSave==='function'){coreListFilterSave('expedition','active');coreListFilterSave('dock','active');coreListFilterSave('quest','active')}
...
test('equipment ledger preserves search and shows filtered count', async ({ page }) => {
  ...
  expect(data.count).toContain('1種類・2個');
  expect(data.count).toContain('全2種類・3個');
...
test('mobile global search lives inside header menu', async ({ page }) => {
  ...
  expect(data.topSearchDisplay).toBe('missing');
...
test('mobile header overflow menu uses touch-friendly action grid', async ({ page }) => {
  ...
  const data=await page.evaluate(async()=>{
    ...
    const menu=document.querySelector('#hdMobileHeaderMenuRow .hd-version-menu')||document.querySelector('.hd-version-menu');
...
test('Home current fleet copies to selected map', async ({ page }) => {
  ...
  await page.addInitScript(() => {
    ...
    localStorage.setItem('harbordesk-kancolle-sync-v1', JSON.stringify({
      syncedAt:Date.now(),ships:1,equipment:1,materials:8,decks:1,
...
test('Home current fleet shows sortie readiness summary', async ({ page }) => {
  ...
  await page.addInitScript(() => {
    sessionStorage.setItem('harbordesk-session-guide-view-v1', JSON.stringify({world:'5',map:'5-5',filter:'map',query:'5-5'}));
    localStorage.setItem('harbordesk-kancolle-sync-v1', JSON.stringify({syncedAt:Date.now(),ships:1,equipment:1,materials:8,decks:1,coverage:{ships:true,equipment:true,resources:true,fleets:true,quests:true,docks:true,sorties:true}}));
    localStorage.setItem('harbordesk-kancolle-fleets-v1', JSON.stringify([
...
test('quick nav ranks categories and context by real usage', async ({ page }) => {
  ...
  await boot(page,errors);
  const data=await page.evaluate(()=>{
    const pinId=(window.hdWSVisibleSections?.('fleet')||[]).map(x=>x.id).find(id=>!['roster','trainingPlanner'].includes(id))||'roster';
    localStorage.setItem('harbordesk-quick-nav-pins-v1',JSON.stringify([pinId]));
    localStorage.setItem('harbordesk-quick-nav-recent-v1',JSON.stringify([
  ...
  expect(data.context[0]).toBe(data.pinId);
