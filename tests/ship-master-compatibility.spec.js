...
test('ConningTowerLite compatible logs import through HAR and generic records', async ({ page }) => {
  const errors = [];
  await boot(page, errors);
        ...
        entries:[
          {
            ...
            request:{url:'https://example.invalid/kcsapi/api_req_map/start?api_token=SECRET'},
            response:{content:{encoding:'base64',text:btoa(JSON.stringify(startPayload))}}
          },
          {
            startedDateTime:'2026-09-19T10:00:10.000Z',
            request:{url:'https://example.invalid/kcsapi/api_req_sortie/battleresult?api_token=SECRET'},
            response:{content:{text:'svdata='+JSON.stringify(battlePayload)}}
          },
          {
            startedDateTime:'2026-09-19T10:00:20.000Z',
            request:{url:'https://example.invalid/kcsapi/api_port/port?api_token=SECRET'},
            response:{content:{text:JSON.stringify(portPayload)}}
          }
        ]
      }
    };

    const parsedHar = window.hdKcParseImport?.(JSON.stringify(har));
    const previewHar = window.hdKcPreviewData?.(parsedHar);
    const first = window.hdKcApplyImport?.(previewHar,{ships:false,equipment:false,resources:false,fleets:false,timers:false,quests:false,sorties:true});
    const second = window.hdKcApplyImport?.(previewHar,{ships:false,equipment:false,resources:false,fleets:false,timers:false,quests:false,sorties:true});
    const sortieRows = JSON.parse(localStorage.getItem('harbordesk-sortie-log-v1')||'[]');

    const generic = {
      logs:[
        {
          url:'https://example.invalid/kcsapi/api_get_member/material?api_token=TOPSECRET&api_verno=1',
          responseBody:'svdata='+JSON.stringify({
            api_result:1,
            api_data:[{api_id:1,api_value:4321}]
          })
        }
      ]
    };
    const parsedGeneric = window.hdKcParseImport?.(JSON.stringify(generic));
    const previewGeneric = window.hdKcPreviewData?.(parsedGeneric);

    const result = {
      endpoint:window.hdKcExternalEndpoint?.('https://example.invalid/kcsapi/api_port/port?api_token=SECRET#x'),
      har:{
        bridgeSource:previewHar?.bridgeSource||'',
        captureId:parsedHar?.captureId||'',
        sources:previewHar?.sources||[],
        sortieStarts:previewHar?.sortieStarts||0,
        battleResults:previewHar?.battleResults||0,
        firstSorties:first?.sorties||0,
        secondSorties:second?.sorties||0,
        logCount:sortieRows.length,
        log:sortieRows[0]||null
      },
      generic:{
        bridgeSource:previewGeneric?.bridgeSource||'',
        sources:previewGeneric?.sources||[],
        materials:previewGeneric?.materials||0,
        value:parsedGeneric?.materials?.get?.(1)?.api_value||0
      },
      bridgeVersion:window.HarborDeskKancolleBridge?.version||0,
      ui:{
        conning:!!document.querySelector('.hd-kc-conning-guide'),
        pasteButton:!!document.querySelector('[data-hd-kc-ct-paste]'),
        fileInput:!!document.getElementById('hdKcConningFile')
      }
    };
  ...
  expect(data.endpoint).toBe('/kcsapi/api_port/port');
  expect(data.har.bridgeSource).toBe('ConningTowerLite互換/HAR');
  expect(data.har.captureId).toMatch(/^ct-/);
  expect(data.har.sources).toEqual([
    '/kcsapi/api_req_map/start',
    '/kcsapi/api_req_sortie/battleresult',
    '/kcsapi/api_port/port'
  ]);
  expect(data.har.sources.join(' ')).not.toContain('api_token');
  expect(data.har.sources.join(' ')).not.toContain('SECRET');
  expect(data.har.sortieStarts).toBe(1);
  expect(data.har.battleResults).toBe(1);
  expect(data.har.firstSorties).toBe(1);
  expect(data.har.secondSorties).toBe(0);
  expect(data.har.logCount).toBe(1);
  expect(data.har.log).toEqual(expect.objectContaining({
    map:'2-3',
    node:'A',
    result:'S',
    source:'kancolle-import',
  ...
  expect(data.generic.bridgeSource).toBe('ConningTowerLite互換/logs');
  expect(data.generic.sources).toEqual(['/kcsapi/api_get_member/material']);
  expect(data.generic.materials).toBe(1);
  expect(data.generic.value).toBe(4321);

  expect(data.bridgeVersion).toBe(1);
  expect(data.ui).toEqual({conning:true,pasteButton:true,fileInput:true});
  expect(errors, `runtime errors: ${errors.join('\\n')}`).toEqual([]);
});
