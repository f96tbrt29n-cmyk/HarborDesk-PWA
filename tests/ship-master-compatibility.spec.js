...
test('Kancolle sync coverage distinguishes captured and zero sections', async ({ page }) => {
  ...
  const data=await page.evaluate(()=>{
    ...
    return {
      ...
      ok:box?.querySelectorAll('.hd-kc-coverage-chips > .ok').length||0,
      missing:box?.querySelectorAll('.hd-kc-coverage-chips > .missing').length||0
    };
  });
  expect(data.text).toContain('艦娘');
  expect(data.text).toContain('任務 取得済み・0');
  expect(data.text).toContain('入渠 取得済み・0');
  expect(data.text).toContain('出撃 取得済み・0');
  expect(data.ok).toBe(7);
...
test('global search is one tap from header and shows destination hints', async ({ page }) => {
  ...
  const data=await page.evaluate(()=>{
    ...
    const header=document.querySelector('.hd-version-menu [data-hd-gs-open]')||document.getElementById('hdGlobalSearchHeader');
...
test('home reorder controls persist card order and append newly added cards', async ({ page }) => {
  ...
  await page.locator('[data-home-edit-toggle]').click();
  await expect(page.locator('#home')).toHaveClass(/home-editing/);
...
test('core quest and timer filters keep completed items out of the way', async ({ page }) => {
  ...
  const data=await page.evaluate(()=>{
    ...
    coreListFilterSave('expedition','active');coreListFilterSave('dock','active');coreListFilterSave('quest','active');
...
test('home empty states offer direct add actions', async ({ page }) => {
  ...
  await page.evaluate(()=>document.getElementById('questDialog')?.close());
...
test('Kancolle import reports outdated Userscript version', async ({ page }) => {
  ...
  const data=await page.evaluate(async()=>{
    ...
    const current=script.match(/@version\s+([0-9.]+)/)?.[1]||'';
    return {text:el?.textContent||'',link:el?.querySelector('a')?.getAttribute('href')||'',cls:el?.className||'',current};
  });
  expect(data.text).toContain('v1.0.7');
  expect(data.current).not.toBe('');
  expect(data.text).toContain(`v${data.current}`);
...
test('mobile global search lives inside header menu', async ({ page }) => {
  ...
  expect(data.topSearch).toBe(false);
...
test('Kancolle sync shows compact baseline success toast', async ({ page }) => {
  const errors=[];
  await boot(page,errors);
  const data=await page.evaluate(()=>{
...
test('mobile header overflow menu uses touch-friendly action grid', async ({ page }) => {
  ...
  const data=await page.evaluate(async()=>{
    ...
    const menu=document.querySelector('#hdMobileHeaderMenuRow .hd-version-menu')||document.querySelector('.hd-header-more .hd-version-menu');
    const cs=menu?getComputedStyle(menu):null;
    const buttons=menu?[...menu.querySelectorAll('button')].filter(b=>getComputedStyle(b).display!=='none'):[];
  ...
  expect(data.display).toBe('grid');
  const columnCount=data.columns.trim()?data.columns.trim().split(/\s+/).length:0;
...
test('Home current fleet shows sortie readiness summary', async ({ page }) => {
  ...
  await page.addInitScript(() => {
    ...
    sessionStorage.setItem('harbordesk-session-guide-view-v1', JSON.stringify({world:'5',map:'5-5',filter:'map',query:'5-5'}));
    localStorage.setItem('harbordesk-kancolle-sync-v1', JSON.stringify({
      syncedAt:now,ships:1,equipment:1,materials:8,decks:1,quests:0,docks:0,
      coverage:{ships:true,equipment:true,resources:true,fleets:true,quests:true,docks:true,sorties:true}
    }));
    localStorage.setItem('harbordesk-kancolle-fleets-v1', JSON.stringify([
      {deckId:1,name:'第1艦隊',syncedAt:now,mission:[0,0,0,0],ships:[{name:'加賀改',level:94,nowHp:70,maxHp:79,cond:55,gear:'烈風 / 彩雲'}]}
    ]));
    localStorage.setItem('harbordesk-ship-roster-v1', JSON.stringify([{id:'1',name:'加賀改',level:94,tags:[]}]));
    localStorage.setItem('harbordesk-custom-fleets-v1', JSON.stringify({'5-5':[
      {id:'fleet-test',name:'ゲーム同期｜第1艦隊',ships:[{ship:'加賀改',gear:'烈風 / 彩雲',nowHp:70,maxHp:79,cond:55}],source:'kancolle-import',sourceDeckId:1,sourceSyncedAt:now}
    ]}));
  ...
  await boot(page,errors);
  await page.evaluate(()=>window.renderHomeDashboard?.());
  const text=await page.locator('[data-home-sortie-open]').first().textContent();
...
test('sortie readiness warnings link to resolution targets', async ({ page }) => {
  ...
  await page.addInitScript(() => {
    ...
    sessionStorage.setItem('harbordesk-session-guide-view-v1', JSON.stringify({world:'5',map:'5-5',filter:'map',query:'5-5'}));
    localStorage.setItem('harbordesk-custom-fleets-v1', JSON.stringify({'5-5':[{
      id:'fleet-warning-links',name:'ゲーム同期｜第1艦隊',source:'kancolle-import',sourceDeckId:1,sourceSyncedAt:syncedAt,
      ships:[{ship:'加賀改',gameShipId:1,masterId:84,level:99,gear:'',nowHp:10,maxHp:40,cond:32}]
    }]}));
    localStorage.setItem('harbordesk-sortie-selection-v1', JSON.stringify({'5-5':'fleet-warning-links'}));
  });
  await boot(page,errors);
  await page.evaluate(()=>{
  ...
  const rows=await page.evaluate(()=>[...document.querySelectorAll('#hdSortieReadiness [data-hd-sortie-action]')].map(x=>({
    action:x.dataset.hdSortieAction,label:x.textContent.trim(),row:x.closest('.hd-sortie-next,.hd-sortie-auto-row')?.textContent||''
  })));
...
test('mobile update banner clears the two-row dock', async ({ page }) => {
  const errors=[];
  await page.setViewportSize({width:390,height:844});
  await boot(page,errors);
  const data=await page.evaluate(()=>{
...
test('compact peek survives database rerender', async ({ page }) => {
  const errors=[];
  await page.addInitScript(() => {
    sessionStorage.setItem('harbordesk-session-shipdb-view-v1',JSON.stringify({compact:true}));
    sessionStorage.setItem('harbordesk-session-equip-catalog-view-v1',JSON.stringify({compact:true}));
  ...
  const data=await page.evaluate(()=>{
    window.hdEnsureShipDatabase?.();window.hdEnsureEquipmentCatalog?.();
    let ship=document.querySelector('#hdShipDbList .hd-shipdb-card');
    let equip=document.querySelector('#hdEquipCatalogList .hd-equip-ref-card');
    ship?.dispatchEvent(new MouseEvent('click',{bubbles:true}));
    equip?.dispatchEvent(new MouseEvent('click',{bubbles:true}));
    const shipKey=ship?.dataset.hdShipdbPeekKey||'',equipKey=equip?.dataset.hdEquipPeekKey||'';
    window.hdRenderShipDatabase?.();window.hdRenderEquipmentCatalog?.();
    ship=[...document.querySelectorAll('#hdShipDbList .hd-shipdb-card')].find(x=>x.dataset.hdShipdbPeekKey===shipKey);
    equip=[...document.querySelectorAll('#hdEquipCatalogList .hd-equip-ref-card')].find(x=>x.dataset.hdEquipPeekKey===equipKey);
    return {
      shipKey,equipKey,
      shipOpen:!!ship?.classList.contains('hd-peek'),
      equipOpen:!!equip?.classList.contains('hd-peek')
    };
  });
  expect(data.shipKey).not.toBe('');
  expect(data.equipKey).not.toBe('');
...
test('home warns when external backup is missing or stale and exports from warning', async ({ page }) => {
  ...
  const data=await page.evaluate(async()=>{
    ...
    return {
      ...
      fresh:{due:!!freshInfo?.due,label:freshInfo?.label,alert:freshAlert,metricText:metric?.textContent||'',metricShare:metric?.hasAttribute('data-ph-share-backup')||false}
    };
  });
  ...
  expect(data.fresh.metricShare).toBe(true);
  expect(errors, `runtime errors: ${errors.join('\\n')}`).toEqual([]);
});
