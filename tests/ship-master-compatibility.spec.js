...
test('sortie readiness warnings link to resolution targets', async ({ page }) => {
  ...
  await page.evaluate(()=>{
    ...
    const fleet=window.hdKcCopyFleetToCustom?.(1,'5-5');
    if(fleet?.id)window.hdKcSelectFleetForSortie?.('5-5',fleet.id);
    document.querySelector('[data-map-tab="mine"]')?.click();
    window.hdRenderSortieReadiness?.();
  });
  await page.waitForFunction(() => {
    const rows=[...document.querySelectorAll('#hdSortieReadiness [data-hd-sortie-action]')].map(x=>({
      action:x.dataset.hdSortieAction||'',
      label:x.textContent||'',
      row:x.closest('.hd-sortie-auto-row,.hd-sortie-next')?.textContent||''
    }));
    return rows.some(x=>x.action==='kancolleImport') &&
      rows.some(x=>x.action==='home'&&x.row.includes('耐久')) &&
      rows.some(x=>x.action==='home'&&x.row.includes('疲労')) &&
      rows.some(x=>x.action==='tab:gear');
  });
