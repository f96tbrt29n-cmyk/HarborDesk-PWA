...
async function boot(page) {
  ...
  await expect.poll(() => page.evaluate(() => window.HD_MODULE_STATUS?.['./sortie-session.js'] || ''), { timeout: 30000 }).toBe('ok');
  await expect.poll(() => page.evaluate(() => window.HD_MODULE_STATUS?.['./sortie-log.js'] || ''), { timeout: 30000 }).toBe('ok');
}
