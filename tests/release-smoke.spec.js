...
test('release smoke: passive workspace reveal never steals explicit攻略 navigation', async ({ page }) => {
  const errors = [];
  await boot(page, errors);
  await page.waitForFunction(() =>
  ...
  await expect(page.locator('#equipmentBook')).toBeHidden();
  expect(errors).toEqual([]);
});
...
test('release smoke: updater uses GitHub main as release truth and exposes publish lag', async ({ page }) => {
  ...
  expect(source).toContain("const HD_APP_VERSION='1.0.434'");
  expect(source).toContain("const HD_APP_BUILD=434");
...
test('release smoke: build version cache-busts core and runtime assets', async ({ page }) => {
  ...
  for (const asset of ['advanced-tools.js', 'kancolle-import.js', 'equipment-catalog.js', 'home-dashboard.js', 'update-manager.js']) {
    expect(data.index).toContain(`${asset}?v=434`);
  }
  expect(data.updater).toContain("const HD_APP_BUILD=434");
  ...
  expect(data.sw).toContain("harbordesk-pwa-v434");
...
test('release smoke: recovery page preserves local data while clearing app caches', async ({ page }) => {
  ...
  expect(source).toContain('const BUILD=434');
...
test('release smoke: map攻略 critical assets are cache-busted', async ({ page }) => {
  ...
  const data = await page.evaluate(() => {
    ...
    const requiredScripts = [
      'map-details.js?v=434',
      'map-images.js?v=434',
      'map-tabs.js?v=434',
      'map-interactive.js?v=434',
      'map-advanced-data.js?v=434'
    ];
    const requiredStyles = [
      'map-details.css?v=434',
      'map-tabs.css?v=434',
      'map-images.css?v=434',
      'map-interactive.css?v=434'
    ];
    return {
