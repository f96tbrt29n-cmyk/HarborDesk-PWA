...
test('release smoke: updater uses GitHub main as release truth and exposes publish lag', async ({ page }) => {
  ...
  expect(source).toContain("const HD_APP_VERSION='1.0.418'");
  expect(source).toContain("const HD_APP_BUILD=418");
...
test('release smoke: build version cache-busts core and runtime assets', async ({ page }) => {
  ...
  for (const asset of ['advanced-tools.js', 'kancolle-import.js', 'equipment-catalog.js', 'home-dashboard.js', 'update-manager.js']) {
    expect(data.index).toContain(`${asset}?v=418`);
  }
  expect(data.updater).toContain("const HD_APP_BUILD=418");
  ...
  expect(data.sw).toContain("harbordesk-pwa-v418");
...
test('release smoke: recovery page preserves local data while clearing app caches', async ({ page }) => {
  ...
  expect(source).toContain('const BUILD=418');
...
test('release smoke: map攻略 critical assets are cache-busted', async ({ page }) => {
  ...
  const data = await page.evaluate(() => {
    ...
    const requiredScripts = [
      'map-details.js?v=418',
      'map-images.js?v=418',
      'map-tabs.js?v=418',
      'map-interactive.js?v=418',
      'map-advanced-data.js?v=418'
    ];
    const requiredStyles = [
      'map-details.css?v=418',
      'map-tabs.css?v=418',
      'map-images.css?v=418',
      'map-interactive.css?v=418'
    ];
    return {
