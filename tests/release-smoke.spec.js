...
test('release smoke: enhanced equipment catalog preserves reset and compact peek state', async ({ page }) => {
  ...
  const data = await page.evaluate(() => {
    ...
    const openAfter=!!card?.classList.contains('hd-peek');
    const search=document.getElementById('hdEquipCatalogSearch');
    if(search)search.value='電探';
    window.hdRenderEquipmentCatalog?.();
    const activeReset=document.querySelector('.hd-equip-search [data-hd-equip-reset]');
    return {
      initialResetDisabled:!!reset?.disabled,
      key,openBefore,openAfter,
      activeReset:activeReset?.classList.contains('is-active')||false,
      activeResetDisabled:!!activeReset?.disabled
    };
  });
