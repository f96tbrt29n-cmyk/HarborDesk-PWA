...
test('release smoke: hidden completed strategy goal can be undone without losing other progress', async ({ page }) => {
  ...
  await page.locator('[data-home-guide-pending]').click();
  const gear = page.locator('details.home-guide-group[data-group="gear"]');
  await gear.locator('summary').click();
  const gearName = await gear.locator('.home-guide-step').first().locator('strong').textContent();
  await gear.locator('.home-guide-step').first().locator('[data-home-guide-toggle]').click();
  const group = page.locator('details.home-guide-group[data-group="map"]');
  const first = group.locator('.home-guide-step').first();
  const name = await first.locator('strong').textContent();
  await first.locator('[data-home-guide-toggle]').click();
  await expect(group.locator('.home-guide-step').filter({ hasText: name })).toHaveCount(0);
  await expect(page.locator('#homeGuideCount')).toContainText('目標 2/');
  await page.locator('#hdToastRegion .hd-toast-action').click();
  await expect(group.locator('.home-guide-step').filter({ hasText: name })).toHaveCount(1);
  await expect(page.locator('#homeGuideCount')).toContainText('目標 1/');
  await expect(gear.locator('.home-guide-step').filter({ hasText: gearName })).toHaveCount(0);
});
...
test('release smoke: expedition recommendations follow selected resource hourly efficiency', async ({ page }) => {
  await boot(page);
  await page.evaluate(() => window.hdWSShowElement('expeditionOptimizer', true));
  await page.locator('[data-opt-exp-minutes="240"]').click();
  const cards = page.locator('#hdOptExpRows article');
  const ranked = async resource => {
    await page.locator('#hdOptExpResource').selectOption(resource);
    const rows = await cards.evaluateAll(items => items.map(item => ({
      name: item.querySelector('strong')?.textContent,
      rate: Number(item.dataset.optExpHourly),
      label: item.querySelector('b')?.textContent
    })));
    expect(rows.length).toBeGreaterThan(1);
    rows.forEach((row, index) => {
      expect(row.name).toContain(`${index + 1}位`);
      expect(row.label).toContain(resource === 'ammo' ? '弾薬' : 'ボーキ');
      if(index)expect(rows[index - 1].rate).toBeGreaterThanOrEqual(row.rate);
    });
    return rows[0].name;
  };
  const ammoFirst = await ranked('ammo');
  const bauxiteFirst = await ranked('bauxite');
  expect(bauxiteFirst).not.toBe(ammoFirst);
});
...
test('release smoke: game sync persists exact gear slots for comparison', async ({ page }) => {
  ...
  await boot(page, errors);
  const source = await page.evaluate(async () => fetch('./kancolle-import.js', { cache:'no-store' }).then(r => r.text()));
  expect(source).toContain('gameGearSlots:parsed.slotItems.size||parsed.completeSlotItems?gameGearSlots');
  expect(source).toContain('gameGearExpansion:parsed.slotItems.size||parsed.completeSlotItems?gameGearExpansion');
...
test('release smoke: updater uses GitHub main as release truth and exposes publish lag', async ({ page }) => {
  ...
  expect(source).toContain(`const HD_APP_VERSION='1.0.${APP_BUILD}'`);
  expect(source).toContain(`const HD_APP_BUILD=${APP_BUILD}`);
...
test('release smoke: build version cache-busts core and runtime assets', async ({ page }) => {
  ...
  for (const asset of ['advanced-tools.js', 'kancolle-import.js', 'equipment-catalog.js', 'home-dashboard.js', 'update-manager.js']) {
    expect(data.index).toContain(`${asset}?v=${APP_BUILD}`);
  }
  expect(data.updater).toContain(`const HD_APP_BUILD=${APP_BUILD}`);
  ...
  expect(data.sw).toContain(`harbordesk-pwa-v${APP_BUILD}`);
...
test('release smoke: recovery page preserves local data while clearing app caches', async ({ page }) => {
  ...
  expect(source).toContain(`const BUILD=${APP_BUILD}`);
...
test('release smoke: map攻略 critical assets are cache-busted', async ({ page }) => {
  ...
  const data = await page.evaluate(build => {
    ...
    const requiredScripts = [
      `map-details.js?v=${build}`,
      `map-images.js?v=${build}`,
      `map-tabs.js?v=${build}`,
      `map-interactive.js?v=${build}`,
      `map-advanced-data.js?v=${build}`
    ];
    const requiredStyles = [
      `map-details.css?v=${build}`,
      `map-tabs.css?v=${build}`,
      `map-images.css?v=${build}`,
      `map-interactive.css?v=${build}`
    ];
    return {
...
test('release smoke: real iPhone flow opens map攻略 tools', async ({ page }) => {
  ...
  expect(src).toBe(`app.js?v=${APP_BUILD}`);
  expect(errors).toEqual([]);
});
...
test('release smoke: diagnostics center runtime stylesheet is loaded', async ({ page }) => {
  ...
  expect(data.href).toContain(`diagnostics-center.css?v=${APP_BUILD}`);
