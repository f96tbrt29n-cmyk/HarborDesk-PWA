...
document.addEventListener('click',e=>{
 ...
 const buildLink=e.target.closest?.('[data-hd-shipdb-acquire-build]');if(buildLink){const query=buildLink.dataset.hdShipdbAcquireBuild||'',mode=buildLink.dataset.hdShipdbBuildMode==='large'?'large':'normal';if(typeof window.hdConstructionOpenSearch==='function'){window.hdConstructionOpenSearch(query,mode,true);return}hdEnsureConstructionSection?.();hdConstructionMode=mode;const input=document.getElementById('hdConstructionSearch');if(input)input.value=query;renderConstructionDb?.();window.hdWSShowElement?.('constructionDb',true);return}
