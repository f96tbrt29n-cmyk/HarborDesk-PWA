const HD_APP_VERSION='1.0.23';
const HD_APP_BUILD=23;
...
async function hdCheckForUpdate(showResult=false){
  hdEnsureUpdateUI();
  const btn=document.getElementById('hdUpdateCheck');
  try{
    if(btn){btn.disabled=true;btn.textContent='確認中…'}
    const latest=await hdFetchLatestVersion();
    const newer=Number(latest.build||0)>HD_APP_BUILD;
    const banner=document.getElementById('hdUpdateBanner');
    const text=document.getElementById('hdUpdateText');
    if(newer){
    ...
    if(showResult)alert('更新情報を確認できなかったよ。通信状態を確認してもう一度試してね。');
  }finally{
    if(btn){btn.disabled=false;btn.textContent='更新確認'}
  }
}
