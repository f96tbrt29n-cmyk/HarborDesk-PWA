...
function hdCCJump(id){const el=document.getElementById(id);if(el){el.scrollIntoView({behavior:'smooth',block:'start'});history.replaceState(null,'',`#${id}`)}else location.hash=id}
...
function hdCCTimers(){
 ...
 const builds=hdCCJson('harbordesk-construction-timers-v1',[]);for(const x of Array.isArray(builds)?builds:[]){const end=Number(x.endsAt||x.endAt||x.finishAt);if(end>hdCCNow())out.push({...x,endsAt:end,kind:'建造',target:'constructionDatabase'})}
