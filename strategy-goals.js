...
function hdStrategyFocusHtml(map,state,rows,path=[]){
 const focus=hdStrategyFocusRows(map,state,rows);
 if(!focus.length)return '';
 return `<div class="hd-strategy-focus"><strong>${path.length?`${hdStrategyEsc(map)} 向けに並行して進める準備`:"次に進める準備"}</strong><small>${path.length?`まず ${hdStrategyEsc(path[0])} を攻略。こちらは並行して準備できます。`:"未完了の任務・編成・装備・育成から表示。"}達成状況はゲーム画面でも確認してください。</small>${focus.map(({row,existing})=>`<div class="hd-strategy-preview-item"><div><b>${hdStrategyEsc(row.title)}</b><small>${hdStrategyEsc(row.detail)}</small></div><button type="button" class="ghost small" ${existing?`data-hd-strategy-focus-group="${hdStrategyEsc(row.category)}" data-hd-strategy-focus-key="${hdStrategyEsc(row.sourceKey)}"`:`data-hd-strategy-import="${hdStrategyEsc(row.sourceKey)}"`}>${existing?'リストで見る':'目標に追加'}</button></div>`).join('')}</div>`;
}
function hdStrategyPreview(map,state){
 if(!map)return '';
 const rows=hdStrategyCandidates(map),existing=new Set(state.custom.filter(x=>x.map===map).map(x=>x.sourceKey)),pending=rows.filter(x=>!existing.has(x.sourceKey)&&!hdStrategyCandidateReady(x,state)),path=hdStrategyPendingPath(map,state),focus=hdStrategyFocusHtml(map,state,rows,path),nextFocus=path.length&&map!=='5-6'?hdStrategyFocusRows(path[0],state):[],grouped={unlock:[],nav:[],gear:[],level:[],quest:[]};
 ...
 return `<div class="hd-strategy-preview"><div class="hd-strategy-preview-head"><strong>${hdStrategyEsc(map)} の目標候補</strong><button type="button" class="primary small" data-hd-strategy-import-all="${hdStrategyEsc(map)}" ${pending.length?'':'disabled'}>未追加の目標をまとめて追加（${pending.length}件）</button></div>${path.length?`<div class="hd-strategy-preview-item"><div><b>前提海域の攻略順：${hdStrategyEsc(path.join(' → '))} → ${hdStrategyEsc(map)}</b><small>${map==='5-6'?'5-5 は今月のゲージ破壊をゲーム画面で確認。':'クリア記録をもとに未攻略の前提海域を表示。'} 次は ${hdStrategyEsc(path[0])} を確認。${nextFocus.length?`まず ${hdStrategyEsc(path[0])} の準備：${nextFocus.map(x=>hdStrategyEsc(x.row.title)).join('、')}。`:''}</small></div><button type="button" class="ghost small" data-hd-strategy-next-map="${hdStrategyEsc(path[0])}">${hdStrategyEsc(path[0])} を見る →</button></div>`:''}${focus}<small>保存編成・装備台帳・育成計画と攻略情報から作成。達成はゲーム画面でも確認してください。</small>${sections.map(([id,label])=>`<details class="hd-strategy-preview-section" ${id==='unlock'||id==='nav'?'open':''}><summary>${label}<span>${grouped[id].length}件</span></summary>${grouped[id].length?grouped[id].map(x=>`<div class="hd-strategy-preview-item"><div><b>${hdStrategyEsc(x.title)}</b><small>${hdStrategyEsc(x.detail)}</small></div><button type="button" class="ghost small" data-hd-strategy-import="${hdStrategyEsc(x.sourceKey)}" ${existing.has(x.sourceKey)||(x.source==='unlock'&&hdStrategyCandidateReady(x,state))?'disabled':''}>${existing.has(x.sourceKey)?'追加済み':hdStrategyCandidateReady(x,state)?x.source==='unlock'?'クリア記録済み':'達成済みを表示':'追加'}</button></div>`).join(''):'<p class="muted">現在の登録データから候補はありません。</p>'}</details>`).join('')}<div class="hd-strategy-preview-tools"><button type="button" class="ghost small" data-hd-strategy-procurement="${hdStrategyEsc(map)}">装備計画に不足種別を登録</button><button type="button" class="ghost small" data-home-jump="trainingPlanner">育成計画へ</button><a href="https://wikiwiki.jp/kancolle/%E4%BB%BB%E5%8B%99" target="_blank" rel="noopener">任務の前提をWikiで確認 ↗</a></div></div>`;
}
