...
function rosterScheduleRender(delay=90){clearTimeout(shipRosterRenderTimer);shipRosterRenderTimer=setTimeout(renderShipRoster,Math.max(0,Number(delay)||0))}
...
function renderShipRoster(){
 ...
 const active=document.querySelector('[data-roster-filter].active')?.dataset.rosterFilter||'all',sourceFilter=document.querySelector('[data-roster-source].active')?.dataset.rosterSource||'all';
 ...
 const rows=allRows.filter(x=>{const sourceOk=sourceFilter==='all'||(sourceFilter==='sync'?x.source==='kancolle-import':x.source!=='kancolle-import');return sourceOk&&(active==='all'||(x.tags||[]).includes(active))&&(!q||`${x.name} ${x.remodel||''} ${(x.tags||[]).join(' ')} ${x.memo||''}`.toLowerCase().includes(q))}).sort(sorter);
 ...
 const reset=document.querySelector('[data-roster-reset]'),dirty=!!q||active!=='all'||sourceFilter!=='all'||sort!=='level';if(reset){reset.disabled=!dirty;reset.classList.toggle('is-active',dirty)}
 ...
 const chips=[];if(q)chips.push('検索: '+q);if(sourceFilter!=='all')chips.push('登録: '+(sourceFilter==='sync'?'ゲーム同期':'手動'));if(active!=='all')chips.push('タグ: '+active);if(sort!=='level')chips.push('並び: '+sortLabel);
 ...
 host.innerHTML=rows.length?rows.map(x=>{const sourceBadge=x.source==='kancolle-import'?'<span class="roster-source-badge sync">ゲーム同期</span>':'<span class="roster-source-badge manual">手動</span>';const identity=typeof hdShipIdentityStatus==='function'?hdShipIdentityStatus({name:x.name,masterId:x.masterId}):null,image=typeof hdShipImageThumbHtml==='function'?hdShipImageThumbHtml(identity?.master?{id:identity.master.id,name:identity.master.name}:x.masterId?{id:x.masterId,name:x.name}:x.name,'roster-thumb'):'';const identityBadge=identity?.master?`<span class="roster-master-id ${identity.status}">MASTER ID ${identity.master.id}</span>`:x.masterId?`<span class="roster-master-id invalid">MASTER ID ${rosterEsc(x.masterId)}?</span>`:'<span class="roster-master-id missing">MASTER ID 未設定</span>';const mismatch=identity?.status==='mismatch'?`<div class="roster-identity-warning"><span>登録名「${rosterEsc(x.name)}」 / マスター名「${rosterEsc(identity.canonical)}」</span><button type="button" class="ghost small" data-roster-master-sync="${x.id}">公式名へ同期</button></div>`:'';return `<article class="roster-card"><div class="roster-card-head">${image}<div class="roster-card-main"><div><strong>${rosterEsc(x.name)}</strong><div class="muted">${x.type?`${rosterEsc(x.type)} ・ `:''}${x.level?`Lv.${rosterEsc(x.level)}`:''}${x.remodel?` ・ ${rosterEsc(x.remodel)}`:''}</div></div><div class="roster-actions"><button class="ghost small" data-roster-edit="${x.id}">編集</button><button class="ghost small" data-roster-delete="${x.id}">削除</button></div></div></div><div class="roster-badges">${sourceBadge}${identityBadge}${(x.tags||[]).map(t=>`<span>${rosterEsc(t)}</span>`).join('')}</div>${mismatch}${x.gear?`<div class="roster-note"><b>装備:</b> ${rosterEsc(x.gear)}</div>`:''}${x.memo?`<div class="roster-note"><b>メモ:</b> ${rosterEsc(x.memo)}</div>`:''}</article>`}).join(''):emptyHtml;
...
function initShipRoster(){
 ...
 const view=rosterViewLoad(),savedFilter=SHIP_TAGS.includes(view.filter)?view.filter:'all',savedSource=['all','sync','manual'].includes(view.sourceFilter)?view.sourceFilter:'all';
 const filters=document.getElementById('shipRosterFilters');if(filters){let source=document.getElementById('shipRosterSourceFilters');if(!source){source=document.createElement('div');source.id='shipRosterSourceFilters';source.className='roster-source-filters';filters.insertAdjacentElement('beforebegin',source)}source.innerHTML=[['all','すべて'],['sync','同期済み'],['manual','手動']].map(([v,label])=>`<button class="guide-chip ${v===savedSource?'active':''}" data-roster-source="${v}">${label}</button>`).join('');filters.innerHTML=['all',...SHIP_TAGS].map(x=>`<button class="guide-chip ${x===savedFilter?'active':''}" data-roster-filter="${x}">${x==='all'?'すべて':x}</button>`).join('')}
 const search=document.getElementById('shipRosterSearch');if(search){search.value=String(view.query||'');search.addEventListener('input',()=>{rosterViewSave({query:search.value});rosterScheduleRender()})}
...
document.addEventListener('click',e=>{
 if(e.target.closest('[data-empty-roster-add]')){openShipRosterDialog();return}
 if(e.target.closest('[data-empty-roster-reset]')){document.querySelector('[data-roster-reset]')?.click();return}
 const reset=e.target.closest('[data-roster-reset]');if(reset){
  ...
  document.querySelectorAll('[data-roster-filter]').forEach(b=>b.classList.toggle('active',b.dataset.rosterFilter==='all'));document.querySelectorAll('[data-roster-source]').forEach(b=>b.classList.toggle('active',b.dataset.rosterSource==='all'));
  rosterViewSave({query:'',filter:'all',sourceFilter:'all',sort:'level'});renderShipRoster();return
 }
 ...
 const sf=e.target.closest('[data-roster-source]');if(sf){document.querySelectorAll('[data-roster-source]').forEach(x=>x.classList.remove('active'));sf.classList.add('active');rosterViewSave({sourceFilter:sf.dataset.rosterSource||'all'});renderShipRoster();return}
